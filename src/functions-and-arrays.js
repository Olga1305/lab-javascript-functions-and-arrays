...
function findLongestWord(words) {
  if (words.length < 1) {
    ...
    let itemLength = words[0].length;
    for (let i = 1; i < words.length; i++) {
      if (words[i].length > itemLength) {
...
function averageWordLength(wordsArr) {
  if (wordsArr.length < 1) {
    ...
    let newArray = wordsArr.map(function(item) {
      return item.length;
    });
...
let yourWord = window.prompt("Which word do you want to find?");

function doesWordExist(wordsFind, yourWord) {
  if (wordsFind.length < 1) {
    return false;
  } else if (wordsFind.indexOf(yourWord) === -1) {
    return false;
  } else {
    return true;
  }
